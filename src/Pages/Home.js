...
const deleted = async(data)=>{
  ...
  notify();
    try {
const response=await axiosInstance.delete('/notes/delete-note/'+dataId)
...
const updatePinn=async(data)=>{
  ...
  try {
    const response=await axiosInstance.put('/notes/update-note-pinned/'+ dataId,content1,{});
...
useEffect(() => {
  ...
  return (
    <div>
      <Navbar userInfo={userInfo} searchingNote={searchingNote} setallNotes={setallNotes}/>
      <div className='container mx-auto'> 
      <div className='grid grid-cols-3 mt-6 gap-4 ml-5 mr-5'>
        
      {allNotes.map((note) => (<Notecard
       title={note.title} 
...
<div
        ...
        >
            ...
            <Modal isOpen={open} onRequestClose={handleClose} className='w-[40%] max-h-3/4 bg-white border-spacing-0 mx-auto rounded-md mt-[42px] p-5 border  shadow-sm '>
                <>
                <Addeditnotes arrayOfNotes={arrayOfNotes} setarrayOfNotes={setarrayOfNotes}  selectedNote={selectedNote} setallNotes={setallNotes} allNotes={allNotes} setOpen={setOpen}
                 getAllNotes={getAllNotes}></Addeditnotes>
                </>
            </Modal>
        </div>
